...
quant_fila = int(input())
fila = list(map(int, input().split()))
quant_desist = int(input())
saidas_fila = list(map(int, input().split()))
primeiro = True
for saida in saidas_fila:
    fila.remove(saida)
for pessoa in fila:
    if primeiro: #condição para não imprimir espaço a mais ao final (Presentation Error)
        print(pessoa, end="")
        primeiro = False
    else:
        print(" " + str(pessoa), end="")
print() #quebra de linha pra não ter Presentation Error
